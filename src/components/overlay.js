    ...
    setContent = ({ 
        ...
        const template = {}
				for (let key of Object.keys(data)) {
					...
					if (!['title', 'body', 'footer'].includes(key)) 
						delete template[key]
				}
        $('.'+classPrefix).html()
        switch(type) {
						case 'landing':
							template.title = [template.title, ...arrayDefaults(!!data?.subtitle, [`<span class="${classPrefix}-subtitle">${data.subtitle}</span>`])].join('\n')
            ...
            case 'instructions':
							template.body = [
								data.body,
								`<ol class='list-menu'>${data.list.map((val) => `<li class='list-item'>${val}</li>`).join('\n')}</ol>`,
							].join('\n')
							template.footer = `<a class='overlay-footer-button'>${data.footer}</a>`
							break
            case 'help':
							template.body = [
								data.body,
								`<ul class='list-menu'>${data.list.map((val) => `<li class='list-item'>${val}</li>`).join('\n')}</ul>`,
							].join('\n')
        ...
        let result_component = [
					...arrayDefaults(closeButton, `<div class="${classPrefix}-close-button"><i data-eva="close-outline"></i></div>`),
					`<div class="${classPrefix}-container">
					${[
							...arrayDefaults(template.title, `<div class="${classPrefix}-header">${template.title}</div>`),
							...arrayDefaults(template.body,`<div class="overlay-body">${template.body}</div>`),
							...arrayDefaults(template.footer,`<div class="overlay-footer">${template.footer}</div>`),
						].join('\n')}</div>`
				].join('\n')
