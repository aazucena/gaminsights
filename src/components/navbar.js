...
const navbar = ({
    ...
    for (let item of items.sort((a, b) => a.weight - b.weight)) {
        nav_items.push(`<div class="nav-item" id="${item.name}"><i data-eva="${item.icon}"></i></div>`)
    }
    ...
    for (let item of items.sort((a, b) => a.weight - b.weight)) {
        $('#'+item.name).on('click', ($el) => {
            switch(item.name) {
                case 'volume':
									let state_icon = $(`#${item.name} > i > svg`).attr('class').split(' ')[1].replace('eva-', '')
									console.log("🚀 ~ file: navbar.js:46 ~ $ ~ state_icon:", state_icon)
